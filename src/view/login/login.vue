<template>
  <div class="login">
    ...
    <group>
      ...
      <x-input title="密码：" required placeholder="密码" type="password" v-model="data.pwd" @keyup.native.enter="login" v-show="step==1">
        <span slot="label" class="loginIcon">
          <i class="iconfont">&#xe6ec;</i>
        </span>
      </x-input>
...
export default {
  ...
  computed: {
    disabled() {
      if(this.step==1){
        return this.data.uid && this.data.pwd ? false : true;
      }else{
        return this.data.uid && this.sms? false : true;
      }
    }
  },
  methods: {
    ...
    uniLogin() {
      if (this.data.uid && this.data.pwd) {
        this.$API
          .uniLogin(this.data)
    ...
    loginOK(val) {
      ...
      if (!this.getCookie("WeixinOpenid") && this.$store.getters.isWeixin) {
        window.location.href =
          ...
          "/api/OAuth2Redirect/index?refUrl=" +
